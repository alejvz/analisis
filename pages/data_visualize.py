...
def app():
    if 'main_data.csv' not in os.listdir('data'):
        ...
        category = st.selectbox("Seleccionar categoría ", Categorical + Object)
        ...
        categoryObject=st.selectbox("Seleccionar " + (str)(category),unique_Category_val[category])
        st.write(cat_groups[category].get_group(categoryObject).describe())
        colName = st.selectbox("Seleccionar ",Numerical)
